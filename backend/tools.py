...
# # 1. Initialize a free local embedding model
# embeddings = HuggingFaceEmbeddings(model_name="all-MiniLM-L6-v2")


# vector_db = Chroma(persist_directory="./my_coffee_db", embedding_function=embeddings)

# @tool(args_schema=KnowledgeBaseInput, return_direct=True)
# def knowledge_base_search(query: str) -> str:
#     """Search product documentation and FAQs to provide accurate answers about the company products and policies."""
#     # 3. Perform a similarity search on your local data
#     docs = vector_db.similarity_search(query, k=3)
    
#     # Combine results into a single answer
#     context = "\n".join([d.page_content for d in docs])
#     # print(context)
#     return f"Based on our documents: {context}"
